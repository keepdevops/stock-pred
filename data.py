...
def store_in_duckdb(dfs, batch_size=1000, db_path="stocks.db"):
...
def sample_tickers_by_letter(tickers, per_letter=50):
    """Randomly sample tickers for each letter"""
    ticker_groups = group_tickers_by_letter(tickers)
    sampled_tickers = []
...
def show_duckdb_status():
    ...
    try:
        ...
        db_size = os.path.getsize("stocks.db") / (1024*1024)  # Convert to MB
...
def get_current_progress(db_path="stocks.db"):
...
if __name__ == "__main__":
    try:
        ...
        start_letter = get_current_progress()
        ...
        sampled_tickers = sample_tickers_by_letter(tickers, per_letter=100)
        ...
        for letter in sorted(ticker_groups.keys()):
            ...
            letter_failures = process_letter_group(tickers_for_letter, letter)
