...
def store_in_duckdb(dfs, batch_size=1000, db_path="nasdaq_stocks.db"):
...
def show_duckdb_status():
    ...
    try:
        con = duckdb.connect("nasdaq_stocks.db")
        ...
        db_size = os.path.getsize("nasdaq_stocks.db") / (1024*1024)  # Convert to MB
...
def get_current_progress(db_path="nasdaq_stocks.db"):
...
if __name__ == "__main__":
    try:
        ...
        # Get NASDAQ tickers (limited to 1500)
        tickers = get_nyse_tickers(limit=1500)
        ...
        # Sample tickers (maintain max 50 per letter)
        sampled_tickers = sample_tickers(tickers)
