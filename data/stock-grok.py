...
class AIAgent:
    ...
    def __init__(self, db_conn=None):
        ...
        self.learning_rate = 0.001  # Added missing parameter
        self.batch_size = 32
        self.epochs = 50
    ...
    def build_model(self, input_shape):
        ...
        try:
            ...
            self.model = model  # Ensure assignment
...
def initialize_control_panel(main_frame, databases):
    ...
    try:
        ...
        def create_output_area(parent_frame):
            """Create a text output area for logs and results"""
            print("Creating output area...")
            
            # Create output frame
            output_frame = ttk.LabelFrame(parent_frame, text="Results")
            output_frame.pack(fill="both", expand=True, padx=10, pady=5, anchor="s")
            
            # Create text widget with scrollbar
            output_text = tk.Text(output_frame, height=15, width=40, bg="#3E3E3E", fg="white")
        ...
        def on_database_selected(event=None):
            ...
            try:
                ...
                print(f"Error in on_database_selected: {str(e)}")
            ...
            try:
                ...
                try:
                    ...
                    if "ticker" in columns:
                        ...
                        # Populate ticker listbox
                        for ticker in tickers:
                            ticker_listbox.insert(tk.END, ticker)
                            
                        # Select first ticker by default (optional)
                        if tickers:
                            ticker_listbox.selection_set(0)
                            
                except Exception as e:
                    print(f"Error in on_table_selected: {str(e)}")
        ...
        refresh_button = ttk.Button(
            db_frame, 
            text="Refresh", 
            command=lambda: refresh_database_list(db_var, db_combo)
        )
        ...
        ticker_scrollbar = ttk.Scrollbar(
            ticker_listbox_frame,
            orient="vertical",
            command=ticker_listbox.yview
        )
        ...
        # Add buttons for selection control
        ticker_buttons_frame = ttk.Frame(ticker_frame)
        ticker_buttons_frame.pack(fill="x", padx=5, pady=2)
        
        select_all_btn = ttk.Button(
            ticker_buttons_frame,
            text="Select All",
            command=lambda: ticker_listbox.select_set(0, tk.END)
        )
        ...
        clear_btn = ttk.Button(
            ticker_buttons_frame,
            text="Clear",
            command=lambda: ticker_listbox.selection_clear(0, tk.END)
        )
        ...
        ttk.Label(ai_frame, text="Model Architecture:").pack(anchor="w", padx=5)
        ...
        def train_model_handler():
            ...
            # Update status
            status_var.set(f"Training model...")
            ...
            output_text.insert("end", f"Database: {db_name}\n")
            output_text.insert("end", f"Table: {table_name}\n")
            output_text.insert("end", f"Ticker: {ticker}\n\n")
        ...
        def make_prediction_handler():
            ...
            output_text.insert("end", f"Days to predict: {days}\n\n")
            ...
            if figure_global and canvas_global:
                ...
                rows = int(n_tickers ** 0.5) + 1
                cols = (n_tickers // rows) + 1
                ...
                for i, ticker in enumerate(tickers):
                    try:
                        ...
                        if df.empty:
                            output_text.insert("end", f"No data for {ticker}\n")
                            continue
                            
                        df['date'] = pd.to_datetime(df['date'])
                        historical_dates = df['date'].tolist()[-30:]
                        historical_values = df['close'].tolist()[-30:]
                        
                        last_value = historical_values[-1]
                        future_dates = [df['date'].iloc[-1] + pd.Timedelta(days=j+1) for j in range(days)]
                        future_values = [last_value * (1 + np.random.normal(0, 0.01)) for _ in range(days)]
                        
                        # Plot each ticker in subplot
                        ax = figure_global.add_subplot(rows, cols, i+1)
                        ax.plot(historical_dates, historical_values, 'b-', label='Historical')
                        ax.plot(future_dates, future_values, 'r--', label='Predicted')
                        ax.set_title(ticker)
                        ax.legend()
                        figure_global.autofmt_xdate()
                        
                        output_text.insert("end", f"Prediction for {ticker}: {future_values[-1]:.2f}\n")
                        
                    except Exception as e:
                        output_text.insert("end", f"Error predicting for {ticker}: {str(e)}\n")
                
                figure_global.tight_layout()
                canvas_global.draw()
        ...
        return {
            ...
            'ticker_listbox': ticker_listbox,
            'days_combo': days_combo,
            'model_combo': model_combo,
            'epochs_entry': epochs_entry,
            'batch_size_entry': batch_size_entry,
            'learning_rate_entry': learning_rate_entry,
            'sequence_length_entry': sequence_length_entry,
            'train_button': train_button,
            'predict_button': predict_button,
            'load_model_button': load_model_button,
